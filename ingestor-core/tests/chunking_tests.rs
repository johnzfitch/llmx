use llmx_mcp::{ingest_files, IngestOptions};
...
fn deterministic_chunking_across_runs() {
    ...
    let files_a: Vec<llmx_mcp::FileInput> = fixtures
        .iter()
        .map(|(fixture, path)| llmx_mcp::FileInput {
            path: (*path).to_string(),
...
fn ingests_images_as_assets_without_utf8_decode() {
    ...
    assert!(index.warnings.is_empty());
    assert_eq!(index.files[0].kind, llmx_mcp::ChunkKind::Image);
    assert_eq!(index.chunks.len(), 1);
    assert_eq!(index.chunks[0].kind, llmx_mcp::ChunkKind::Image);
...
fn selective_update_keeps_unchanged_paths() {
    ...
    let updated = llmx_mcp::update_index_selective(
        prev,
        vec![llmx_mcp::FileInput {
            path: "docs/b.md".to_string(),
            data: b"# B\n\nNew\n".to_vec(),
            mtime_ms: None,
